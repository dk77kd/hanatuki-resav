...
/* ─── State ─── */
const state = {
  plan: null, planLabel: '', planPrice: 0,
  ...
  discount: null,      // { key, label, amount }
};

/* ─── Constants ─── */
const PLAN_LABELS = {
  planA:   'プランA：アンティーク着物スタジオ撮影',
  planB:   'プランB：アンティーク着物スタジオ＋おでかけ',
  planC:   'プランC：モダンスタイルスタジオ＋そのままおでかけ',
  planOwn: 'お持込プラン：お持込着物スタジオ撮影',
};
const PLAN_PRICES = { planA: 77600, planB: 82000, planC: 78700, planOwn: 74300 };
const EXTRA_SUBJECT_PRICE = 36300;
...
const DISCOUNT_CUTOFF_DATE = '2026-08-31';
const DISCOUNT_OPTIONS = [
  { key: 'off10000',   label: '撮影料金 10,000円OFF',                          amount: 10000 },
  { key: 'kimono_free', label: '訪問着レンタル着付け 無料（通常33,000円 ➔ 0円）', amount: 16500 },
];
...
function goToStep(n) {
  ...
  if (n <= 7) {
    ...
    if (n === 3) goToOptionSubStep(1);
    if (n === 5) renderDiscountOptions();
    if (n === 7) buildConfirmTable();
  }
  window.scrollTo({ top: 0, behavior: 'smooth' });
}

function isDiscountEligible() {
  return !!state.date && state.date <= DISCOUNT_CUTOFF_DATE;
}

function goToStepAfterDate() {
  goToStep(isDiscountEligible() ? 5 : 6);
}

function goBackFromBuyerInfo() {
  goToStep(isDiscountEligible() ? 5 : 4);
}

/* ─── Option sub-step navigation ─── */
function goToOptionSubStep(n) {
  document.querySelectorAll('#step3 .substep').forEach((el, i) => {
    el.classList.toggle('active', i + 1 === n);
  });
  document.querySelectorAll('#step3SubIndicator .substep-dot').forEach((dot, i) => {
...
function validateStep(n) {
  if (n === 1) {
    if (!state.plan) { show('planError'); return false; }
    hide('planError');
  }
  if (n === 2) {
    let ok = true;
    if (!state.memberCount) { show('memberCountError'); ok = false; }
    else hide('memberCountError');
    state.subjects.forEach((s, i) => {
      if (!s.gender) { show(`subjectGenderError-${i}`); ok = false; }
      else hide(`subjectGenderError-${i}`);
      if (!s.age) { show(`subjectAgeError-${i}`); ok = false; }
...
function selectPlan(lbl) {
  document.querySelectorAll('.plan-card').forEach(c => c.classList.remove('selected'));
  lbl.classList.add('selected');
  state.plan = lbl.querySelector('input').value;
  state.planLabel = PLAN_LABELS[state.plan];
  state.planPrice = PLAN_PRICES[state.plan];
  hide('planError');
  updatePriceTotal();
}

/* ─── Step 2: Shooting info ─── */
function toggleMemberCount(btn) {
  document.querySelectorAll('#memberCountGroup .age-toggle').forEach(b => b.classList.remove('selected'));
  btn.classList.add('selected');
  const count = parseInt(btn.dataset.count, 10);
  state.memberCount = btn.dataset.count;
  while (state.subjects.length < count) state.subjects.push({ gender: null, age: null });
  state.subjects = state.subjects.slice(0, count);
  hide('memberCountError');
  renderSubjectList();
...
function renderSubjectList() {
  ...
  state.subjects.forEach((s, i) => {
    ...
    card.innerHTML = `
      <p class="subject-title">ご主役${SUBJECT_NUMS[i]}</p>
      <div class="form-row">
      ...
      <div class="form-row" style="margin-bottom:0">
        ...
        <div class="age-toggle-group">
          <button type="button" class="age-toggle${s.age === '3歳' ? ' selected' : ''}" data-age="3歳" onclick="toggleSubjectAge(this,${i})">3歳</button>
          <button type="button" class="age-toggle${s.age === '5歳' ? ' selected' : ''}" data-age="5歳" onclick="toggleSubjectAge(this,${i})">5歳</button>
          <button type="button" class="age-toggle${s.age === '7歳' ? ' selected' : ''}" data-age="7歳" onclick="toggleSubjectAge(this,${i})">7歳</button>
        </div>
        <p class="error-msg" id="subjectAgeError-${i}">年齢をお選びください</p>
      </div>
    `;
...
function toggleSubjectAge(btn, i) {
  btn.closest('.subject-card').querySelectorAll('[data-age]').forEach(b => b.classList.remove('selected'));
  btn.classList.add('selected');
  state.subjects[i].age = btn.dataset.age;
  hide(`subjectAgeError-${i}`);
}

/* ─── Step 3-1: Family dressing ─── */
...
/* ─── Step 3-2/3: Album + location + sibling ─── */
...
function updatePriceTotal() {
  ...
  const extra    = Math.max(0, state.subjects.length - 1) * EXTRA_SUBJECT_PRICE;
  const discount = state.discount ? state.discount.amount : 0;
  const total    = Math.max(0, base + album + opts + dress + extra - discount);
  const text     = total > 0 ? '¥' + total.toLocaleString('ja-JP') : '—';
...
function isKimonoFreeEligible() {
  return state.dressing.some(d => d.relation === 'mother' && d.selectedOpts && d.selectedOpts['kitsuke_rental_f']);
}

function renderDiscountOptions() {
  const container = document.getElementById('discountOptions');
  if (!container) return;
  const eligibleOpts = DISCOUNT_OPTIONS.filter(o => o.key !== 'kimono_free' || isKimonoFreeEligible());
  if (state.discount && !eligibleOpts.some(o => o.key === state.discount.key)) {
    state.discount = null;
  }
  container.innerHTML = eligibleOpts.map(o => `
    <button type="button" class="option-toggle${state.discount && state.discount.key === o.key ? ' selected' : ''}"
      data-key="${o.key}" onclick="selectDiscount(this)">
      <span class="opt-check">✓</span>
      <span class="opt-label">${o.label}</span>
      <span class="opt-price">−¥${o.amount.toLocaleString('ja-JP')}</span>
    </button>`).join('');
  updatePriceTotal();
}
...
function selectDiscount(btn) {
  const key         = btn.dataset.key;
  const wasSelected = btn.classList.contains('selected');
  document.querySelectorAll('#discountOptions .option-toggle').forEach(b => b.classList.remove('selected'));
  if (wasSelected) {
    state.discount = null;
  } else {
    btn.classList.add('selected');
    state.discount = DISCOUNT_OPTIONS.find(o => o.key === key) || null;
  }
  updatePriceTotal();
}
...
function buildConfirmTable() {
  ...
  const totalPrice = Math.max(0, state.planPrice + state.albumPrice +
    Object.values(state.options).reduce((s, o) => s + o.price, 0) +
    state.dressing.reduce((sum, d) =>
      sum + Object.values(d.selectedOpts).reduce((s, o) => s + o.price, 0), 0) +
    Math.max(0, state.subjects.length - 1) * EXTRA_SUBJECT_PRICE -
    (state.discount ? state.discount.amount : 0));

  const subjectLines = state.subjects
    .map((s, i) => `ご主役${SUBJECT_NUMS[i]}：${s.gender || '—'}　${s.age || '—'}`)
    .join('<br>');

  const rows = [
    ['Plan',      state.planLabel + `（¥${state.planPrice.toLocaleString('ja-JP')}）`],
    ['お子様',    subjectLines],
    ['お支度',    state.dressingCount === 0 ? 'なし' : dressingLines.join('<br>')],
    ['Option',    optionLines.length ? optionLines.join('<br>') : 'なし'],
    ['割引',      state.discount ? `${state.discount.label}（−¥${state.discount.amount.toLocaleString('ja-JP')}）` : 'なし'],
...
【お子様情報】
${state.subjects.map((s, i) => `  ${SUBJECT_NUMS[i]}：${s.gender || '未設定'} / ${s.age || '未設定'}`).join('\n')}
