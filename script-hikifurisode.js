...
const state = {
  ...
  options: {},
  subjects: [{ role: '新婦', height: '' }, { role: '新郎', height: '' }],
  optionQty: {},
  dressingCount: 0,
...
function validateStep(n) {
  if (n === 1) {
    ...
    state.subjects.forEach((s, i) => {
      if (!s.height || !s.height.trim()) { show(`subjectHeightError-${i}`); ok = false; }
      else hide(`subjectHeightError-${i}`);
    });
    return ok;
  }
  if (n === 3) {
...
function renderSubjectList() {
  ...
  state.subjects.forEach((s, i) => {
    ...
    card.innerHTML = `
      <p class="subject-title">${s.role}</p>
      <div class="form-row" style="margin-bottom:0">
        <label class="form-label">身長<span class="badge req">必須</span></label>
        <input type="text" placeholder="例：160cm" value="${s.height || ''}" oninput="updateSubjectHeight(this,${i})">
        <p class="error-msg" id="subjectHeightError-${i}">身長をご入力ください</p>
      </div>
    `;
...
function updateSubjectHeight(input, i) {
  state.subjects[i].height = input.value;
  if (input.value.trim()) hide(`subjectHeightError-${i}`);
}
...
function buildConfirmTable() {
  const optionLines = [
    `引き振袖婚礼 基本撮影料（＋¥${BASE_PRICE.toLocaleString('ja-JP')}）`,
...
【新郎・新婦情報】
${state.subjects.map(s => `  ${s.role}：身長 ${s.height || '未設定'}`).join('\n')}
